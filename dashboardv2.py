...
if option=='Fetch Latest':
    if exists('log-'+str(date.today())+'.csv'):
        ...
        df_new_data = df_new_data.rename(columns={'latitude':'lat','longitude':'long','timestamp':'date','speed':'speed (kmph)'})
        ...
        fig =px.scatter_mapbox(df_new_data,lat='long',lon='lat',hover_name='date',hover_data=['speed (kmph)'],mapbox_style='dark',zoom=12,center={'lat':19.057435,'lon': 83.824615})
...
if date_avl==True:
    col1,col2 = st.columns(2)
    with col1:
        normalspeed = [x for x in df_new_data['speed (kmph)'] if x>5 and x<=45 and math.isnan(x)==False]
        m = max([x for x in df_new_data['speed (kmph)'] if math.isnan(x)==False])
        if len(normalspeed)>0:
            a = sum(normalspeed)/len(normalspeed)
        else:
            a=0
        st.plotly_chart(px.bar(x=[m,a],y=['Max Speed','Average Speed'],title='Average Speed v/s Max Speed',labels={'x':'Speed (kmph)','y':'Type'}))

    with col2:
        st.subheader("Maximum Speed Record")
        st.dataframe(df_new_data[df_new_data['speed (kmph)']==m],use_container_width=True)

    col1,col2 = st.columns(2)
    with col1:
        overspeed = [x for x in df_new_data['speed (kmph)'] if x>45 and math.isnan(x)==False]
    ...
    with col2:
        ...
        st.download_button(label='Download',data=df_new_data[df_new_data['speed (kmph)']>45].to_csv().encode('utf-8'),file_name='Bus_Overspeeding_Log.csv')
        st.dataframe(df_new_data[df_new_data['speed (kmph)']>45],use_container_width=True)
    ...
    col1,col2 = st.columns(2)
    with col1:
        idletime = [x for x in df_new_data['speed (kmph)'] if x<5 and math.isnan(x)==False]
        runtim = [x for x in df_new_data['speed (kmph)'] if x>5 and math.isnan(x)==False]
    ...
    with col2:
        ...
        st.download_button(label='Download',data=df_new_data[df_new_data['speed (kmph)']>5].to_csv().encode('utf-8'),file_name='Bus_Activity.csv')
        st.dataframe(df_new_data[df_new_data['speed (kmph)']>5],use_container_width=True)
